const CHECKS = [
  ...
  { id: 'gptbot', label: 'GPTBot allowed', weight: 3, check: (txt) => /User-agent:\s*GPTBot/i.test(txt) && /Allow:\s*\//i.test(txt.split(/User-agent:\s*GPTBot/i)[1].split(/\nUser-agent:/i)[0]) },
  { id: 'anthropic-ai', label: 'anthropic-ai allowed', weight: 3, check: (txt) => /User-agent:\s*anthropic-ai/i.test(txt) && /Allow:\s*\//i.test(txt.split(/User-agent:\s*anthropic-ai/i)[1].split(/\nUser-agent:/i)[0]) },
];
